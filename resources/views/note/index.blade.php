<x-layout>
    <div @class(['mx-auto', 'max-w-5xl', 'px-4', 'py-8', 'sm:px-6', 'lg:px-8'])>
        <header @class(['mb-8', 'flex', 'items-center', 'justify-between'])>
            ...
            <a href="#create" @class(['rounded-full', 'bg-cyan-100', 'px-4', 'py-2', 'text-sm', 'font-medium', 'text-white', 'transition', 'hover:bg-green-700'])>
                + New Note
            </a>
        </header>
            ...
            @forelse($notes as $note)
                <article @class(['rounded-2xl', 'border', 'border-slate-200', 'bg-white', 'p-6', 'shadow-sm', 'transition', 'hover:-translate-y-1', 'hover:shadow-md', 'flex', 'flex-col'])>
                    ...
                    <div @class(['flex-1'])>
                        ...
                        <a href="{{ route('note.show',$note) }}" @class(['text-sm', 'font-semibold', 'text-slate-900', 'hover:text-slate-600'])>
                            View note →
                        </a>
                    </div>
